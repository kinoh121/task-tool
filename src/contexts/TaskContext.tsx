...
import {
  ...
  query, orderBy, serverTimestamp, type Timestamp, writeBatch,
} from 'firebase/firestore';
import { db } from '../firebase';
import { useAuth } from './AuthContext';
import type { Group, List, Task, Priority, TaskStatus } from '../types';
  ...
  const val: TaskContextValue = {
    ...
    addTask: async (data) => {
      ...
      setTasks(prev => {
        ...
        return [...prev, { id, content: data.content || '', detail: data.detail || '', priority: data.priority || 'C', dueDate: data.dueDate ?? null, listId: data.listId || '', status: 'active', order: maxOrder + 1, addedToToday: data.addedToToday ?? false, isTopPriority: false, isSecondPriority: false, completedAt: null, archivedAt: null, createdAt: new Date(), copiedFromId: null }];
      });
      return id;
    },
    updateTask: async (id, data) => setTasks(t => t.map(x => x.id === id ? { ...x, ...data } : x)),
    deleteTask: async (id) => setTasks(t => t.filter(x => x.id !== id)),
    completeTask: async (id) => setTasks(t => t.map(x => x.id === id ? { ...x, status: 'completed', completedAt: new Date(), addedToToday: false, isTopPriority: false, isSecondPriority: false } : x)),
    restoreTask: async (id) => setTasks(t => t.map(x => x.id === id ? { ...x, status: 'active', completedAt: null } : x)),
    archiveTasks: async (ids) => setTasks(t => t.map(x => ids.includes(x.id) ? { ...x, status: 'archived', archivedAt: new Date(), addedToToday: false, isTopPriority: false, isSecondPriority: false } : x)),
    copyTasks: async (ids) => {
      setTasks(prev => {
        const copies = prev.filter(t => ids.includes(t.id)).map(t => ({ ...t, id: newId(), status: 'active' as TaskStatus, addedToToday: true, createdAt: new Date(), archivedAt: null, completedAt: null, copiedFromId: t.id, isTopPriority: false, isSecondPriority: false }));
        return [...copies, ...prev];
      });
    },
    addToToday: async (id) => setTasks(t => t.map(x => x.id === id ? { ...x, addedToToday: true } : x)),
    removeFromToday: async (id) => setTasks(t => t.map(x => x.id === id ? { ...x, addedToToday: false, isTopPriority: false, isSecondPriority: false } : x)),
    ...
    duplicateTask: async (id) => {
      setTasks(prev => {
        ...
        const copy: Task = { ...src, id: String(++demoIdSeq), order: maxOrder + 1, createdAt: new Date(), copiedFromId: id, isTopPriority: false, isSecondPriority: false, addedToToday: false };
        return [...prev, copy];
      });
    },
  };
...
function RealTaskProvider({ children }: { children: React.ReactNode }) {
  const { user } = useAuth();
  const [state, dispatch] = useReducer(reducer, { groups: [], lists: [], tasks: [], loading: true });
  ...
  useEffect(() => {
    ...
    return () => { unsubGroups(); unsubLists(); unsubTasks(); };
  }, [user]);
  ...
  const addTask = async (data: Partial<Task>) => {
    const ref = await addDoc(collection(db, 'users', uid, 'tasks'), {
      ...
      listId: data.listId || '',
      status: 'active',
      addedToToday: data.addedToToday ?? false,
      isTopPriority: false,
  ...
  const updateTask = async (id: string, data: Partial<Task>) => {
    ...
    if (data.addedToToday !== undefined) update.addedToToday = data.addedToToday;
  ...
  const addToToday = async (id: string) => {
    await updateDoc(doc(db, 'users', uid, 'tasks', id), { addedToToday: true });
  };

  const removeFromToday = async (id: string) => {
    await updateDoc(doc(db, 'users', uid, 'tasks', id), {
      addedToToday: false, isTopPriority: false, isSecondPriority: false,
    });
  };
  ...
  const duplicateTask = async (id: string) => {
    ...
    await addDoc(collection(db, 'users', uid, 'tasks'), {
      ...
      order: maxOrder + 1, addedToToday: false,
